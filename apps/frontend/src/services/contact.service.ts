...
import type { Contact, PaginatedResponse, Conversation } from '@/types';

interface ListContactsParams {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: 'name' | 'createdAt' | 'updatedAt' | 'phoneNumber' | 'firstName' | 'lastName' | 'companyName';
  sortOrder?: 'asc' | 'desc';
}
...
class ContactService {
  ...
  async getConversations(
    ...
    const { data } = await api.get<PaginatedResponse<Conversation>>(
      `${CONTACT_API_BASE_URL}/${contactId}/conversations`,
      { params }
    );
    return data;
  }
